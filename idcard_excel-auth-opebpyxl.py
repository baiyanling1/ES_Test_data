...
def get_auth_user(num):
    ...
    # mysql_BIZ.init()
    # 生成csv文件
    # with open("user_data_10000", "a") as csvfile:
    #     writer = csv.writer(csvfile)
    #     writer.writerow(['name', 'msisdn',
    #                           'idcard','group'])
    x = ""
    y = ""
    z = ""
    ...
    wb.save(r'普通用户导入模板-auth-'+str(num)+'.xlsx')
    # mysql_BIZ.commit()
def get_auth_cpe_user(num):
    ...
    for r in range(1, num+1):
        for c in range(1, 13):
            ...
            # show_info()
            # with open("user_data_10.csv", "a") as csvfile:
            # writer = csv.writer(csvfile)
            # writer.writerow([x.join(randname()),z.join(randomnum()),y.join(randid()),str(random.choice(surname))])
            # mysql_BIZ.insert(str(random.randint(10000000000,999999999999)),create_at,str(y.join(randid())),0,z.join(randomnum()),x.join(randname()),0,update_at)
    ...
    wb.save(r'普通用户导入模板-CPE-auth-'+str(num)+'.xlsx')
    # mysql_BIZ.commit()

if __name__ == '__main__':
    get_auth_cpe_user(13)
    get_auth_user(11)
    # wb = openpyxl.Workbook()
    # sheet1 = wb.active
    # sheet1.title = "鉴权终端用户数据"
    ...
    #
    # #表头
    # titles = ('姓名','手机号*','身份证','分组',	'部门','认证方式（0免密认证，1密码认证）*','认证用户名*','证密码','IP地址','IP地址池名称','MAC地址','备注')
    # for index in range(len(titles)):
    #     # write的第一个参数：行，第二个参数：列 第三个参数：内容 第四个参数：样式
    #     sheet1.cell(row=1, column=index + 1).value = titles[index]
    # # show_info()
    ...
    #         # str_id_connt = list()
    #         # str_phone_connt = list()
    # wb.save(r'普通用户导入模板-100000-auth.xlsx')
    # # mysql_BIZ.commit()